...
public final class RandomUtil {
	...
	/**
	 * Returns a start and end position (start > end possible) which is at most
	 * maxEnd * maxPortion long.
	 * 
	 * @param maxEnd
	 *            maximal value for start and end
	 * @param maxPortion
	 *            maximal size with respect to maximal size
	 * @return a start and end position (start > end possible) which is at most
	 *         maxEnd * maxPortion long
	 */
	public static int[] getStartAndEnd(int maxEnd, double maxPortion) {
		int start = ThreadLocalRandom.current().nextInt(maxEnd);
		int end = ThreadLocalRandom.current().nextInt(maxEnd + 1);

		int length = (start < end) ? (end - start) : (maxEnd - start + end);
		while (((length + 0.0) / maxEnd) > maxPortion) {
			start = (start + 1) % maxEnd;
			length--;
		}
