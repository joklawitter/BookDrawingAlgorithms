...
public class GraphUtils {
	...
	public static boolean isBipartite(Graph graph) {
		...
		stack.add(graph.getVertexByIndex(0));
		partition[0] = 0;

		while (!stack.empty()) {
			Vertex u = stack.pop();
			for (Vertex v : u.getNeighbors()) {				
				if (partition[v.getIndex()] == partition[u.getIndex()]) return false;
				if (partition[v.getIndex()] == -1) {
					partition[v.getIndex()] = 1 - partition[u.getIndex()];
					stack.add(v);
				}
			}
		}
